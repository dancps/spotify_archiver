...
def get_playlist_tracks(playlist_id, token, limit=100, offset=0):
    ...
    url = "https://api.spotify.com/v1/playlists/{}/tracks?limit={}&offset={}".format(
...
def get_tracks_list(playlist_id, token):
    ...
    range_steps = range(
        calculate_request_steps(get_playlist_tracks(playlist_id, token), n=100)
    )
...
def get_playlist_cover(
    playlist_id, token
):  # Ref: https://developer.spotify.com/console/get-playlist-images/
    """
...
def get_playists(user, token, limit=50, offset=0):
    ...
    playlists_url = (
        "https://api.spotify.com/v1/users/{}/playlists?limit={}&offset={}".format(
            user, limit, offset
        )
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Saves the playlists of the user into jsons and csvs."
    )
    parser.add_argument("-u", "--user", required=True, help="set the user")
    parser.add_argument(
        "-t",
        "--token_file",
        default=".auth/tokens.txt",
        help="specifies the text file containing user token",
    )
    parser.add_argument(
        "--user_only",
        action="store_true",
        help="store only playlists owned by specified user",
    )
    parser.add_argument(
        "-f",
        "--force",
        action="store_true",
        help="forces to save(this will overwrite the file if it already exists)",
    )
    ...
    with open(token_path, "r") as tok:
        ...
        for step, plst_step in enumerate(plst_lst):
            with open(
                os.path.join("json", "playlists_part_{}.json".format(step)), "w"
            ) as fl:
                fl.write(plst_step.text)

        id_list = [
            (playlists["id"], playlists["name"], playlists["owner"]["id"])
            for pl_step in plst_lst
            for playlists in pl_step.json()["items"]
        ]

        print(
            "Getting "
            + colored(args.user, "blue", attrs=["bold"])
            + " and "
            + colored("others", "red", attrs=["bold"])
            + " playlists."
        )
        progbar = tqdm(
            id_list,
        ...
        for id, name, owner in progbar:
            ...
            if owner == args.user:
                progbar.set_description(
                    colored("{}".format(show_name[:20]), "blue", attrs=["bold"])
                )

                # Creates the json playlist folder
                playlist_json_path = os.path.join(
                    "json", "playlists", args.user, name.replace("/", "_")
                )
                if not (args.force) and os.path.isdir(playlist_json_path):
                    continue  # deletes after
                ...
                for playlist_part_indx, playlist_part in enumerate(playlist):
                    playlist_json = os.path.join(
                        playlist_json_path,
                        "{}_{}.json".format(name.replace("/", "_"), playlist_part_indx),
                    )
                    if not (args.force) and os.path.isfile(playlist_json):
                        continue
                ...
                music = [
                    ...
                    for mus in play.json()["items"]
                ]
                album = [
                    mus["track"]["album"]["name"]
                    for play in playlist
                    for mus in play.json()["items"]
                ]
                artist = [
                    ";".join([artist_s["name"] for artist_s in mus["track"]["artists"]])
                    for play in playlist
                    for mus in play.json()["items"]
                ]
                duration_ms = [
                    mus["track"]["duration_ms"]
                    for play in playlist
                    for mus in play.json()["items"]
                ]
                added_at = [
                    mus["added_at"] for play in playlist for mus in play.json()["items"]
                ]
                added_by = [
                    mus["added_by"]["id"]
                    for play in playlist
                ...
                playlist_path = os.path.join(
                    "playlists", args.user, name.replace("/", "_")
                )
                ...
                wget.download(
                    cover_url,
                    out=os.path.join(
                        playlist_path, "{}_cover.png".format(name.replace("/", "_"))
                    ),
                    bar="",
                )
                ...
                playlist_csv_path = os.path.join(
                    playlist_path, name.replace("/", "_") + ".csv"
                )
                ...
                progbar.set_description(
                    colored("{}".format(show_name[:20]), "red", attrs=["bold"])
                )

                # Creates the json playlist folder
                if not (args.user_only):
                    playlist_json_path = os.path.join(
                        "json", "playlists", "other", name.replace("/", "_")
                    )
                    if not (args.force) and os.path.isdir(playlist_json_path):
                        continue  # deletes after
                    if not (os.path.isdir(playlist_json_path)):
                    ...
                    for playlist_part_indx, playlist_part in enumerate(playlist):
                        playlist_json = os.path.join(
                            playlist_json_path,
                            "{}_{}.json".format(
                                name.replace("/", "_"), playlist_part_indx
                            ),
                        )
                        if not (args.force) and os.path.isfile(playlist_json):
                            continue
                    ...
                    artist = [
                        ";".join(
                            [artist_s["name"] for artist_s in mus["track"]["artists"]]
                        )
                        for play in playlist
                        for mus in play.json()["items"]
                    ]
                    ...
                    # Creates a folder to store the csv and cover image
                    playlist_path = os.path.join(
                        "playlists", "other", name.replace("/", "_")
                    )
                    if not (os.path.isdir(playlist_path)):
                        os.makedirs(playlist_path)
